...
def getIp():
    ...
    response=opener.open("http://127.0.0.1:8000", timeout=10)
...
if __name__=="__main__":
  ...
  #搜索百度内所有内容，进行整理
	urlstr=("设计模式之禅")
	url=trans_url_bd("zhihu.com", "site:(zhihu.com)  "+ urlstr)
	opener=makeMyOpener()
	...
	f=open('./urls.txt','a+')
	f.write('%s======='%urlstr)
	for div in soup.findAll('div', attrs={'class':"result c-container "}):
	  print('get the div')
...
#	      s = subprocess.Popen(str("wget -i  %s"%hrefname), stderr=subprocess.PIPE, stdout=subprocess.PIPE, shell=True)
#	      print(str("wget -i  %s"%hrefname))
#	      print(s)
...
#	f.write(data.decode(chardet.detect(data)['encoding']))
#	f.flush()
#	f.close()
